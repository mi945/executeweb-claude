    ...
    >
      ...
      <div className="p-4">
                  ...
                  {showMenu && (
                    <>
                      ...
                      <motion.div
                        ...
                        className="absolute right-0 top-full mt-1 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-40 min-w-[120px]"
                      >
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
        ...
        <h2 className="text-lg font-bold text-gray-900 mb-1 leading-tight">
          {task.title}
          {task.editedAt && (
            <span className="text-xs font-normal text-gray-400 ml-1.5">(edited)</span>
          )}
        </h2>
      ...
      <div className="border-t border-gray-100 bg-gray-50/50">
        <div className="flex items-center divide-x divide-gray-200">
          ...
          {proofCount > 0 && (
            <button
              ...
              className="flex-1 flex items-center justify-center gap-1.5 py-2 text-xs text-gray-500 hover:text-green-600 hover:bg-gray-100/50 transition-colors"
            >
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path
                  strokeLinecap="round"
              ...
              <span>
                {showProofs ? 'Hide' : `${proofCount} ${proofCount === 1 ? 'proof' : 'proofs'}`}
              </span>
            </button>
          )}
        </div>
        ...
        <div className="px-4 pb-3 border-t border-gray-100">
          <TaskComments taskId={task.id} compact userProfile={userProfile} taskCreatorId={task.creator?.id} />
        </div>
